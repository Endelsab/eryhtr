...
//singleStudent
export const singleData = createAsyncThunk('data/fetchData', async (id) => {
	const response = await fetch(`http://localhost:9000/singlestudent/${id}`);
...
export const updateStudent = createAsyncThunk(
	...
	async (id, updatedData) => {
		...
		const response = await axios.put(
			`http://localhost:9000/student/update/${id}`,
			updatedData,
		);
...
const dataSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			singleData(singleData.pending, (state) => {
				state.status = "loading";
			})
			singleData(singleData.fulfilled, (state, action) => {
				state.status = "succeeded";
				state.studentdatas = action.payload;
			})
			singleData(singleData.rejected, (state, action) => {
				state.status = "failed";
				state.error = action.error.message;
			});
